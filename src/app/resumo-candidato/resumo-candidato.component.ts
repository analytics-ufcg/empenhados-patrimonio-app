...
export class ResumoCandidatoComponent implements OnInit {
  ...
  texto() {
    this.dataService.candidatoSelecionado.subscribe(
      data => (this.candidato = data)
    );
    this.isCandidatoSelecionado = true;
    this.determinaCrescimento(
      this.candidato.patrimonio_eleicao_1,
      this.candidato.patrimonio_eleicao_2
    );
    this.urlBens1 = this.candidatoService.getListaBensURL(
